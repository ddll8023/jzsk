...
public class ChapterServiceImpl implements ChapterService {
    ...
    public ChapterVO createChapter(ChapterDTO chapterDTO) {
        logger.info("创建章节，请求参数：{}", chapterDTO);
        try {
            // 业务逻辑
            logger.info("创建章节成功，章节ID：{}", chapter.getId());
            return convertToVO(chapter);
        } catch (Exception e) {
            logger.error("创建章节失败，请求参数：{}", chapterDTO, e);
            throw e;
        }
    }
}
...
// 方法入口
logger.info("方法名开始执行，参数：{}", param);

// 方法出口
logger.info("方法名执行成功，结果：{}", result);

// 异常日志
logger.error("方法名执行失败，参数：{}", param, e);
```
