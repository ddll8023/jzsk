...
import com.travel.pojo.dto.ScenicSpotCreateDTO;
import com.travel.pojo.vo.ApiResult;
...
**示例**：
```java
@Tag(name = "景点管理", description = "提供景点查询和管理功能")
@RestController
...
// 推荐：依赖全局异常处理
@GetMapping("/{id}")
public ResponseEntity<ApiResult<ScenicSpotVO>> getScenicSpotById(@PathVariable Long id) {
    ScenicSpotVO scenicSpot = scenicSpotService.getScenicSpotById(id);
    return ResponseEntity.ok(ApiResult.success(scenicSpot, "查询成功"));
}
```
...
```java
@PostMapping
public ResponseEntity<ApiResult<ScenicSpotVO>> createScenicSpot(
        @Parameter(description = "景点信息", required = true)
        @Valid @RequestBody ScenicSpotCreateDTO request) {
    // ...
}
```
 ...
 * 权限：公开
 */
@Operation(summary = "查询启用景点列表", description = "根据分类ID查询该分类下所有启用景点")
@GetMapping("/enabled")
public ResponseEntity<ApiResult<List<ScenicSpotVO>>> getEnabledScenicSpotList(
...
public interface ScenicSpotService {
    ...
    @Operation(summary = "根据ID查询景点", description = "查询指定ID的景点信息")
    ScenicSpotVO getScenicSpotById(Long id);
    
    @Operation(summary = "分页查询景点", description = "按条件分页查询景点列表")
    PageResultVO<ScenicSpotVO> getScenicSpotList(int page, int size, ScenicSpotQueryDTO queryDTO);
    
    @Operation(summary = "创建景点", description = "创建新景点，仅管理员可访问")
    ScenicSpotVO createScenicSpot(ScenicSpotCreateDTO scenicSpotCreateDTO);
    
    @Operation(summary = "更新景点", description = "更新指定ID的景点信息，仅管理员可访问")
    ScenicSpotVO updateScenicSpot(Long id, ScenicSpotUpdateDTO scenicSpotUpdateDTO);
    
    @Operation(summary = "删除景点", description = "删除指定ID的景点，仅管理员可访问")
...
public class ScenicSpotServiceImpl implements ScenicSpotService {
    ...
    public ScenicSpotVO createScenicSpot(ScenicSpotCreateDTO scenicSpotDTO) {
        // 验证景点分类存在
        ScenicCategory category = scenicCategoryMapper.selectById(scenicSpotDTO.getCategoryId());
        if (category == null) {
            throw new BusinessException("景点分类不存在");
        }
        
        // 创建景点
        ScenicSpot scenicSpot = new ScenicSpot();
        scenicSpot.setCategoryId(scenicSpotDTO.getCategoryId());
        scenicSpot.setName(scenicSpotDTO.getName());
        scenicSpot.setRegion(scenicSpotDTO.getRegion());
        scenicSpot.setEnabled(scenicSpotDTO.getEnabled());
        scenicSpot.setCreateTime(LocalDateTime.now());
        scenicSpotMapper.insert(scenicSpot);
        
        // 如果有标签列表，则补充写入标签 JSON
        if (scenicSpotDTO.getTags() != null && !scenicSpotDTO.getTags().isEmpty()) {
            scenicSpot.setTagsJson(objectMapper.writeValueAsString(scenicSpotDTO.getTags()));
...
```java
@Override
public ScenicSpotVO updateScenicSpot(Long id, ScenicSpotUpdateDTO scenicSpotDTO) {
    // 参数校验
    if (id == null) {
        throw new IllegalArgumentException("景点ID不能为空");
    }
    if (scenicSpotDTO == null) {
        throw new IllegalArgumentException("景点信息不能为空");
    }
    
    // 业务校验
    ScenicSpot existingScenicSpot = scenicSpotMapper.selectById(id);
    if (existingScenicSpot == null) {
        throw new BusinessException("景点不存在");
    }
...
public class ScenicSpotServiceImpl implements ScenicSpotService {
    ...
    @Override
    public ScenicSpotVO createScenicSpot(ScenicSpotCreateDTO scenicSpotDTO) {
        log.info("创建景点，请求参数：{}", scenicSpotDTO);
        try {
            // 业务逻辑
            log.info("创建景点成功，景点ID：{}", scenicSpot.getId());
            return convertToVO(scenicSpot);
        } catch (Exception e) {
            log.error("创建景点失败，请求参数：{}", scenicSpotDTO, e);
            throw e;
        }
    }
}
 ...
 * @param id 景点ID
 * @return 景点VO
 * @throws BusinessException 景点不存在时抛出
 */
@Override
public ScenicSpotVO getScenicSpotById(Long id) {
    // ...
...
**必须使用XML文件方式进行数据库操作，禁止使用注解方式**：
- ❌ 禁止使用 `@Insert`、`@Delete`、`@Update`、`@Select` 等注解
- ❌ 禁止在Mapper接口上使用SQL注解
- ✅ 必须使用 `XxxMapper.xml` 文件定义SQL语句
- ✅ 所有SQL语句必须在XML中统一管理
...
public interface ScenicSpotMapper {
    ...
    List<ScenicSpot> selectByCondition(ScenicSpotQueryDTO queryDTO);

    List<ScenicSpot> selectAllEnabledByCategoryId(@Param("categoryId") Long categoryId);
}
```
...
<mapper namespace="com.travel.mapper.ScenicSpotMapper">
    ...
    <!-- 基础结果映射 -->
    <resultMap id="BaseResultMap" type="com.travel.pojo.entity.ScenicSpot">
    ...
    <sql id="Base_Column_List">
        id, category_id, name, region, enabled, create_time, update_time
    </sql>
...
<insert id="batchInsert" useGeneratedKeys="true" keyProperty="id">
    INSERT INTO scenic_spot (category_id, name, region, enabled, create_time, update_time)
    VALUES
    <foreach collection="list" item="item" separator=",">
        (#{item.categoryId}, #{item.name}, #{item.region}, #{item.enabled}, 
...
// Service层
@Override
public PageResultVO<ScenicSpotVO> getScenicSpotList(int page, int size, ScenicSpotQueryDTO queryDTO) {
    PageHelper.startPage(page, size);
    List<ScenicSpot> list = scenicSpotMapper.selectByCondition(queryDTO);

    // 使用静态方法快速构建（推荐）
    return PageResultVO.restPage(list, scenicSpot -> convertToVO(scenicSpot));
}
```
...
<insert id="insert" useGeneratedKeys="true" keyProperty="id">
    INSERT INTO scenic_spot (category_id, name, region, enabled, create_time)
    VALUES (#{categoryId}, #{name}, #{region}, #{enabled}, #{createTime})
</insert>
```
...
<update id="deleteLogically">
    UPDATE scenic_spot SET deleted = 1 WHERE id = #{id}
</update>

<!-- 查询时排除已删除数据 -->
<select id="selectById" resultMap="BaseResultMap">
    SELECT * FROM scenic_spot WHERE id = #{id} AND deleted = 0
</select>
```
...
```java
@Schema(name = "景点创建请求", description = "创建新景点的请求参数")
@Data
public class ScenicSpotCreateDTO {
    
    @Schema(description = "所属分类ID", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "分类ID不能为空")
    private Long categoryId;
    
    @Schema(description = "景点名称", example = "西湖", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "景点名称不能为空")
    @Size(max = 100, message = "景点名称长度不能超过100个字符")
    private String name;
    
    @Schema(description = "所属地区", example = "杭州", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "所属地区不能为空")
    @Size(max = 100, message = "所属地区长度不能超过100个字符")
    private String region;
...
```java
// 创建专用
@Schema(name = "景点创建请求", description = "创建新景点")
@Data
public class ScenicSpotCreateDTO {
    // 创建所需的字段
}
...
// 更新专用
@Schema(name = "景点更新请求", description = "更新景点信息")
@Data
public class ScenicSpotUpdateDTO {
...
```java
@Schema(description = "行程明细（JSON格式）", example = "{\"day1\":[\"西湖\",\"灵隐寺\"]}")
...
```java
@Schema(description = "行程明细配置")
private TravelItineraryDTO itinerary;

@Schema(description = "行程明细")
@Data
public class TravelItineraryDTO {
    @Schema(description = "天数标识")
    private String day;
    
    @Schema(description = "景点列表")
    private List<String> scenicSpotNames;
}
```
...
- 包路径：`com.travel.pojo.vo`
- 类名：`PascalCase + VO`

### 7.3 基础结构规范

```java
@Schema(name = "景点VO", description = "景点视图对象")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class ScenicSpotVO {
    ...
    @Schema(description = "景点名称", example = "西湖")
    private String name;
    
    @Schema(description = "所属地区", example = "杭州")
    private String region;
