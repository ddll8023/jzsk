<template>
  <div id="div1">
    <div v-if="!onlyUnprocessed" id="bread">
    ...
    <div id="div-header1">
      ...
      <download-excel v-if="!onlyUnprocessed" class="export-excel-wrapper" :data="PIList" :fields="json_fields" name="预警信息报告.xls"
    ...
    <div id="div-main" :style="onlyUnprocessed ? 'height:70vh;' : ''">
      <el-table :data="filteredPIList" border stripe style="width: 100%" :header-cell-style="onlyUnprocessed ? { background: '#cfe2f3', color: '#222', fontSize: '18px', height: '48px' } : { background: '#cfe2f3', color: '#606266' }" :row-style="onlyUnprocessed ? { fontSize: '18px', height: '44px' } : {}" :height="onlyUnprocessed ? '65vh' : null">
        ...
        <el-table-column v-if="!onlyUnprocessed" fixed="right" label="操作" align="center" width="160">
          <template slot-scope="scope">
            <el-button size="mini" :type="scope.row.status === '未解除' ? 'primary' : 'info'"
              :disabled="scope.row.status === '已解除'" @click="showEditDialog(scope.row.id)">
              {{ scope.row.status === '已解除' ? '已解除' : '解除预警' }}
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div v-if="!onlyUnprocessed" id="div-footer">
  ...
  computed: {
    filteredPIList() {
      if (this.onlyUnprocessed) {
        return this.PIList.filter(item => item.status === '未处理');
      }
      return this.PIList;
    }
  },
