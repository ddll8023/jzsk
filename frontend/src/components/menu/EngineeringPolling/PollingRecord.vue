          ...
          <template slot-scope="scope">
            <div v-if="scope.row.image">
              <div v-for="(img, index) in getLimitedImages(scope.row.image)" :key="index"
                ...
                <img :src="img" style="width: auto; height: 50px; margin-right: 5px;" alt="图片">
              </div>
            ...
            <el-form-item label="图片:" prop="image">
              <div style="display: flex; flex-wrap: wrap; align-items: center;">
                <div v-for="(url, index) in imageList2" :key="index" class="image-container"
                  ...
                  <img :src="url" @click="showBigImage(index)"
                ...
                <el-dialog ref="elDialog" :visible.sync="dialogVisible" width="70%" custom-class="my-custom-dialog">
                  <div class="dialog-body">
                    ...
                    <img :src="currentBigImageUrl" style="width: 100%; max-height: 600px;">
...
export default {
  ...
  this.editForm = res.data
  // 拼接完整图片URL
  this.imageList2 = this.editForm.image
    ? this.editForm.image.split(';')
        .map(img => img.trim())
        .filter(img => !!img)
        .map(img => {
          if (/^https?:\/\//.test(img)) return img;
          return this.photo_url + img.replace(/^\//, '');
        })
    : []
