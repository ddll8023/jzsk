  ...
  <div id="div1">
    <div id="bread">
      <el-breadcrumb separator-class="el-icon-arrow-right" style="padding-top:5px; padding-left:10px">
        ...
        <el-breadcrumb-item :to="{ path: '/home/usermanagedata' }">用户管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
...
export default {
  ...
  methods: {
    //获取数据列表
    async getUserList() {
      const { data: res } = await this.$http.get('/user/list-data', {
    ...
    async getCharacterList() {
      const { data: res } = await this.$http.get('/role/list-data', {
        params: this.queryinfo
      })
      if (res.code !== 200) {
        return this.$message.error('获取数据列表失败')
