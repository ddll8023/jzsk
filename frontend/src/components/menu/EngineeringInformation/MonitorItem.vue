<template>
    <div id="div1">
        <div id="div-header">
            <span style="align-self:center; margin-left:20px; font-size:14px;">测项名称：</span>
            <el-select v-model="name" clearable placeholder="请选择" style="align-self:center;" @change="getMIList"
                @clear="getMIList">
            ...
            <el-button type="primary" round icon="iconfont icon-icon-test" @click="importDialogVisible = true"
                style="align-self:center;margin-right:10px">导入</el-button>
            <el-button type="primary" round icon="iconfont icon-icon-test" @click="exportExcel"
                style="align-self:center;margin-right:10px">导出</el-button>
        </div>

        <div id="div-main">
            <el-table :data="MIList" border stripe height="100%" style="width: 100%"
                ...
                <el-table-column prop="number" label="测项编号" align="center">
                </el-table-column>
                <el-table-column prop="name" label="测项名称" align="center">
                </el-table-column>
                <el-table-column prop="unit" label="测项单位" align="center">
                </el-table-column>
        ...
        <!-- 新增对话框 -->
        <el-dialog title="新增测项信息" :visible.sync="addDialogVisible" width="900px" @close="addDialogClosed" center
            :close-on-press-escape="false" :close-on-click-modal="false">
            <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="150px" size="small">
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="测项编号:" prop="number">
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="测项单位:" prop="unit">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="addMI">确 定</el-button>
                <el-button @click="addDialogVisible = false">取 消</el-button>
            </span>
        </el-dialog>

        <!-- 编辑对话框 -->
        <el-dialog title="编辑测项信息" :visible.sync="editDialogVisible" width="900px" @close="editDialogClosed" center
            :close-on-press-escape="false" :close-on-click-modal="false">
            ...
            <!-- 底部按钮 -->
            <span slot="footer">
                <el-button type="primary" @click="editMI">确 定</el-button>
        ...
        <el-dialog :visible.sync="importDialogVisible" width="800px" :modal="false" :close-on-press-escape="false"
            ...
            <el-upload class="upload-demo" drag ref="upload" accept=".xlsx,.xls" name="file"
                action="/measuring-item/import-excel" :file-list="fileList" :auto-upload="false" :on-change="fileChange"
        ...
        <!-- 导出表格 -->
        <el-table id="table" :data="AllMIList" border stripe height="100%" style="width: 100%;display:none;"
            :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
            <el-table-column type="index" label="序号" width="50px" align="center" :index="table_index">
            </el-table-column>
            <el-table-column prop="number" label="测项编号" align="center" width="100px">
            </el-table-column>
            <el-table-column prop="name" label="测项名称" align="center" width="140px">
            </el-table-column>
            <el-table-column prop="unit" label="测项单位" align="center" width="150px">
            </el-table-column>
        </el-table>

        <!-- 导出数据模板 -->
        <el-table id="tableFormwork" :data="ExampleMI" border stripe height="100%" style="width: 100%;display:none;"
            :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
            <el-table-column type="index" label="序号" width="50px" align="center" :index="table_index">
            </el-table-column>
            <el-table-column prop="number" label="测项编号" align="center" width="100px">
...
export default {
    ...
    data() {
        return {
            ...
            //添加表单的验证规则对象
            addFormRules: {
                number: [{ required: true, message: '请输入测项编号', trigger: 'blur' }],
                name: [{ required: true, message: '请输入测项名称', trigger: 'blur' }],
                unit: [{ required: false, message: '请输入测项单位', trigger: 'blur' }]
            },
            //查询到的信息对象
            editForm: {},
            //编辑表单的验证规则对象
    ...
    methods: {
        ...
        async getMIList() {
            ...
            const { data: res } = await this.$http.get('/measuring-item/list', {
                params: {
                    name: this.name,
            ...
            this.MIList = res.data.records
            this.total = res.data.total
        },
        //获取不分页数据列表
        async getAllMIList() {
            const { data: res } = await this.$http.get('/measuring-item/export-excel')
            if (res.code !== 200) {
                return this.$message.error('获取数据列表失败')
            }
            this.AllMIList = res.data
        },
        //获取所有监测站名称
        async getAllNames() {
        ...
        async httpRequest(param) {
            ...
            let url = '/measuring-item/import-excel'
            let config = {
                headers: {
