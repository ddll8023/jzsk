  ...
  <div id="div1">
    <div id="bread">
      <el-breadcrumb separator-class="el-icon-arrow-right" style="padding-top: 5px; padding-left: 10px">
        ...
        <el-breadcrumb-item :to="{ path: '/home/waterdistributor' }">分水口</el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <div id="div-header1">
      <div style="align-self:center;margin-left:10px">
        <el-input placeholder="请输入分水口名称搜索" v-model="input" class="input-with-select" clearable @clear="getWDList">
          <el-button slot="append" icon="el-icon-search" @click="searchWD"></el-button>
        </el-input>
      </div>
    ...
    <div id="div-main">
      <el-table :data="WDList" border stripe height="100%" style="width: 100%"
        :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
        <el-table-column type="index" label="分水口编号" width="100px" align="center" :index="table_index">
        </el-table-column>
        <el-table-column prop="name" label="分水口名称" align="center">
        </el-table-column>
        <el-table-column prop="county" label="所属区县" align="center">
        </el-table-column>
        <el-table-column prop="town" label="所属乡镇" align="center">
        </el-table-column>
        <el-table-column prop="village" label="所属村庄" align="center">
        </el-table-column>
        <el-table-column prop="longitude" label="分水口经度" align="center">
        </el-table-column>
        <el-table-column prop="latitude" label="分水口纬度" align="center">
        </el-table-column>
        <el-table-column prop="waterSupply" label="所属供水工程" align="center">
        </el-table-column>
        ...
        <el-table-column prop="rtuCode" label="分水口RTU编码" align="center">
        </el-table-column>
    ...
    <!-- 新增对话框 -->
    <el-dialog title="新增分水口" :visible.sync="addDialogVisible" width="800px" @close="addDialogClosed" center
      :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="145px" size="small">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="分水口名称:" prop="name">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="分水口经度:" prop="longitude">
              <el-input v-model="addForm.longitude"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="分水口纬度:" prop="latitude">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="分水口RTU编码:" prop="rtuCode">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addWD">确 定</el-button>
        <el-button @click="addDialogVisible = false">取 消</el-button>
      </span>
    </el-dialog>

    <!-- 编辑对话框 -->
    <el-dialog title="编辑分水口信息" :visible.sync="editDialogVisible" width="800px" @close="editDialogClosed" center
      :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="145px" class="demo-ruleForm"
        size="small ">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="分水口名称:" prop="name">
      ...
      <!-- 底部按钮 -->
      <span slot="footer">
        <el-button type="primary" @click="editWD">确 定</el-button>
    ...
    <el-dialog :visible.sync="importDialogVisible" width="800px" :modal="false" :close-on-press-escape="false"
      ...
      <el-upload class="upload-demo" drag ref="upload" accept=".xlsx,.xls" name="file"
        action="/water-distributor/import-excel" :file-list="fileList" :auto-upload="false" :on-change="fileChange"
    ...
    <!-- 导出表格 -->
    <el-table id="table" :data="AllWDList" border stripe height="100%" style="width: 100%;display:none;"
      :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
      <el-table-column type="index" label="分水口编号" width="100px" align="center" :index="table_index">
      </el-table-column>
      <el-table-column prop="name" label="分水口名称" align="center" width="150px">
      </el-table-column>
      <el-table-column prop="county" label="所属区县" align="center" width="100px">
      </el-table-column>
      <el-table-column prop="town" label="所属乡镇" align="center" width="100px">
      </el-table-column>
      <el-table-column prop="village" label="所属村庄" align="center" width="100px">
      </el-table-column>
      <el-table-column prop="longitude" label="分水口经度" align="center" width="120px">
      </el-table-column>
      <el-table-column prop="latitude" label="分水口纬度" align="center" width="120px">
      </el-table-column>
      <el-table-column prop="waterSupply" label="所属供水工程" align="center" width="120px">
      </el-table-column>
      <el-table-column prop="date" label="建站年月" align="center" width="120px">
      </el-table-column>
      <el-table-column prop="manageUnit" label="管理单位" align="center" width="150px">
      </el-table-column>
      <el-table-column prop="address" label="地址" align="center" width="180px">
      </el-table-column>
      <el-table-column prop="rtuCode" label="分水口RTU编码" align="center" width="150px">
      </el-table-column>
      <el-table-column prop="note" label="备注" align="center">
      </el-table-column>
    </el-table>
    ...
    <!-- 导出数据模板 -->
    <el-table id="tableFormwork" :data="ExampleWD" border stripe height="100%" style="width: 100%;display:none;"
      :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
      <el-table-column type="index" label="分水口编号" width="100px" align="center" :index="table_index">
      </el-table-column>
      <el-table-column prop="name" label="分水口名称" align="center" width="150px">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addFormRules: {
        name: [
          { required: true, message: '请输入分水口名称', trigger: 'blur' },
          { validator: checkname, trigger: 'blur' }
        ],
        county: [
          { required: true, message: '请输入分数区县', trigger: 'blur' }
        ],
        town: [
        ...
        village: [
          { required: true, message: '请输入村庄', trigger: 'blur' },
          { validator: checkname, trigger: 'blur' }
        ],
        longitude: [
          { required: true, message: '请输入分口水经度', trigger: 'blur' },
          {
        ...
        latitude: [
          { required: true, message: '请输入分口水纬度', trigger: 'blur' },
          {
        ...
        rtuCode: [
          { required: true, message: '请输入分水口RTU编码', trigger: 'blur' },
  ...
  methods: {
    //获取数据列表
    async getWDList() {
      const { data: res } = await this.$http.get('/water-distributor/list', {
    ...
    async getAllWDList() {
      const { data: res } = await this.$http.get(
        '/water-distributor/export-excel'
      )
      if (res.code !== 200) {
        return this.$message.error('获取数据列表失败')
      }
      this.AllWDList = res.data
    },
    //获取编辑数据对话框
    async showEditDialog(id) {
      const { data: res } = await this.$http.get(
        '/water-distributor/info/' + id
      )
      if (res.code !== 200) {
    ...
    async searchWD() {
      const { data: res } = await this.$http.get(
        '/water-distributor/search-list-by-name',
        {
          params: {
            name: this.input,
    ...
    async httpRequest(param) {
      ...
      let url = '/water-distributor/import-excel'
      let config = {
        headers: {
