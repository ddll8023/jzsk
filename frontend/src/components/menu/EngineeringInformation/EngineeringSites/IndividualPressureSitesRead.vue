  ...
  <div id="div1">
    <div id="bread">
      <el-breadcrumb separator-class="el-icon-arrow-right" style="padding-top: 5px; padding-left: 10px">
        ...
        <el-breadcrumb-item :to="{ path: '/home/individualpressuresites' }">单独压力站点</el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <div id="div-header1">
      <div style="align-self:center;margin-left:10px">
        <el-input placeholder="请输入单独压力测站搜索" v-model="input" class="input-with-select" clearable @clear="getIPSList">
          <el-button slot="append" icon="el-icon-search" @click="searchIPS"></el-button>
        </el-input>
      </div>
    ...
    <div id="div-main">
      <el-table :data="IPSList" border stripe height="100%" style="width: 100%"
        ...
        <el-table-column prop="name" label="压力测站名称" align="center" min-width="120px">
        </el-table-column>
        ...
        <el-table-column prop="measuringStationsElements" label="测站要素" align="center" min-width="100px">
        </el-table-column>
        <el-table-column prop="splitSiteCode" label="拆分站点编码" align="center" min-width="120px">
        </el-table-column>
        ...
        <el-table-column label="预警指标范围" class="column" align="center">
          <el-table-column prop="upUpLimit" label="上上限" width="100" align="center">
          </el-table-column>
          <el-table-column prop="upLimit" label="上限" width="100" align="center">
          </el-table-column>
          <el-table-column prop="lowLimit" label="下限" width="100" align="center">
          </el-table-column>
          <el-table-column prop="lowerLimit" label="下下限" width="100" align="center">
          </el-table-column>
        </el-table-column>
    ...
    <!-- 新增对话框 -->
    <el-dialog title="新增单独压力站点" :visible.sync="addDialogVisible" width="800px" @close="addDialogClosed" center
    ...
    <!-- 编辑对话框 -->
    <el-dialog title="编辑单独压力站点" :visible.sync="editDialogVisible" width="800px" @close="editDialogClosed" center
      :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="150px" class="demo-ruleForm"
        size="small">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="压力测站名称:" prop="name">
      ...
      <!-- 底部按钮 -->
      <span slot="footer">
        <el-button type="primary" @click="editIPS">确 定</el-button>
    ...
    <el-dialog :visible.sync="importDialogVisible" width="800px" :modal="false" :close-on-press-escape="false"
      ...
      <el-upload class="upload-demo" drag ref="upload" accept=".xlsx,.xls" name="file"
        action="/individual-flow-sites/import-excel" :file-list="fileList" :auto-upload="false" :on-change="fileChange"
    ...
    <!-- 导出表格 -->
    <el-table id="table" :data="AllIPSList" border stripe height="100%" style="width: 100%;display:none;"
      :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
      <el-table-column type="index" label="序号" width="60px" align="center" :index="table_index">
      </el-table-column>
      <el-table-column prop="name" label="压力测站名称" align="center" min-width="120px">
    ...
    <!-- 导出数据模板 -->
    <el-table id="tableFormwork" :data="ExampleIPS" border stripe height="100%" style="width: 100%;display:none;"
      :header-cell-style="{ background: '#cfe2f3', color: '#606266' }">
      <el-table-column type="index" label="序号" width="60px" align="center" :index="table_index">
      </el-table-column>
      <el-table-column prop="name" label="压力测站名称" align="center" min-width="120px">
  ...
  methods: {
    ...
    async getIPSList() {
      const { data: res } = await this.$http.get(
        '/individual-pressure-sites/list',
        {
      ...
      this.IPSList = res.data.records
      this.total = res.data.total
    },
    //获取不分页数据列表
    async getAllIPSList() {
      const { data: res } = await this.$http.get(
        '/individual-pressure-sites/export-excel'
      )
      if (res.code !== 200) {
        return this.$message.error('获取数据列表失败')
      }
      this.AllIPSList = res.data
    },
    //获取编辑数据对话框
    async showEditDialog(id) {
      const { data: res } = await this.$http.get(
        '/individual-pressure-sites/info/' + id
      )
      if (res.code !== 200) {
    ...
    addIPS() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.post(
          '/individual-pressure-sites/save',
          this.addForm
        )
        if (res.code !== 200) {
    ...
    editIPS() {
      this.$refs.editFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await this.$http.post(
          '/individual-pressure-sites/update',
          {
            id: this.editForm.id,
    ...
    async removeById(id) {
      ...
      const { data: res } = await this.$http.post(
        '/individual-pressure-sites/delete/' + id
      )
      if (res.code !== 200) {
    ...
    async searchIPS() {
      const { data: res } = await this.$http.get(
        '/individual-pressure-sites/search-list-by-name',
        {
          params: {
            name: this.input,
    ...
    async httpRequest(param) {
      ...
      let url =
        '/individual-pressure-sites/import-excel'
      let config = {
        headers: {
