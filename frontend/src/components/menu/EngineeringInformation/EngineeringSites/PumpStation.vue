  ...
  data() {
    ...
    return {
      ...
      addFormRules: {
        ...
        number: [
          { required: false, message: '请输入机组数量', trigger: 'blur' },
          { pattern: /^[1-9]\d*$/, message: "请输入正整数", trigger: 'blur' }
        ],
        designScale: [
          { required: false, message: '请输入设计规模', trigger: 'blur' },
          { pattern: /^\d+(\.\d+)?$/, message: "请输入非负数", trigger: 'blur' }
        ],
        installedCapacity: [
          { required: false, message: '请输入装机容量', trigger: 'blur' },
          { pattern: /^\d+(\.\d+)?$/, message: "请输入非负数", trigger: 'blur' }
        ],
        lift: [{ required: false, message: '请输入扬程', trigger: 'blur' },
        { pattern: /^\d+(\.\d+)?$/, message: "请输入非负数", trigger: 'blur' }],
        date: [{ required: false, message: '请输入建站年月', trigger: 'blur' }]
      },
      //查询到的信息对象
      editForm: {},
      //编辑表单的验证规则对象
